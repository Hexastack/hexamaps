...
const generate = (addons) => {
      ...
      install(Vue, options) {
        if (!options || !options.editor) {
          (Vue.prototype.entityOnClick = (e, entity) => {
            ...
            addons
              .filter((plugin) => !!plugin.entity.on.click)
              .forEach((plugin) => {
                changes = plugin.entity.on.click(
                  e,
                  changes,
                  {
                    inputs: entity[plugin.name + "EntityIn"],
                    outputs: entity[plugin.name + "EntityOut"],
                    data: entity[plugin.name + "Entity"],
                  },
                  {
                    inputs: entity[plugin.name + "In"],
                    outputs: entity[plugin.name + "Out"],
                    data: entity[plugin.name],
                  },
                  entity.map.data
                );
              });
          ...
          (Vue.prototype.entityOnHover = (e, entity) => {
            let changes = entity.expose.wrap();
            addons
              .filter((plugin) => !!plugin.entity.on.hover)
              .forEach((plugin) => {
        ...
        Vue.prototype.mapOnData = (map) => {
          addons
            .filter((plugin) => !!plugin.map.on.dataChanged)
            .forEach((plugin) => {
              plugin.map.on.dataChanged(
                map.map.config,
                {
                  inputs: map[plugin.name + "In"],
                  outputs: map[plugin.name + "Out"],
                  data: map[plugin.name],
                },
                map.map.data
              );
            });
        ...
        Vue.prototype.mapOnSource = (map) => {
          addons
            .filter((plugin) => !!plugin.map.on.sourceChanged)
            .forEach((plugin) => {
              plugin.map.on.sourceChanged(
                map.map.config,
                {
                  inputs: map[plugin.name + "In"],
                  outputs: map[plugin.name + "Out"],
        ...
        Vue.prototype.mapOnProjection = (map) => {
          addons
            .filter((plugin) => !!plugin.map.on.projectionChanged)
            .forEach((plugin) => {
              plugin.map.on.projectionChanged(
                map.map.config,
                {
                  inputs: map[plugin.name + "In"],
                  outputs: map[plugin.name + "Out"],
  ...
  addons.forEach((plugin) => {
    ...
    plugins.entityComponents = plugins.entityComponents.concat(
      ...
      created() {
        if (plugin.entity.lifeCycle.created)
          plugin.entity.lifeCycle.created(
            this,
            {
              inputs: this[plugin.name + "EntityIn"],
              outputs: this[plugin.name + "EntityOut"],
              data: this[plugin.name + "Entity"],
            },
            {
              inputs: this[plugin.name + "In"],
              outputs: this[plugin.name + "Out"],
              data: this[plugin.name],
            },
            this.map.data
          );
      },
